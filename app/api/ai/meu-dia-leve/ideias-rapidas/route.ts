// app/api/ai/meu-dia-leve/ideias-rapidas/route.ts
...
type Body = {
  ...
  focus?: Focus
  avoidIds?: string[]
  count?: number // opcional (default 3)
}
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await sb
      .from('adm_ideas')
      .select('id,title,short_description,duration_minutes,tags,status,hub')
      .eq('hub', 'meu-dia-leve')
      .ilike('id', 'mdl-ir-%')
      .eq('status', 'published')
    ...
    if (error) {
      // mantém padrão do projeto: status 200 + ok:false
    ...
    const items = shuffle(remaining.slice()).slice(0, count)

    return NextResponse.json(
      { ok: true, meta: { source: 'adm', poolSize, returnedCount: items.length, exhausted, avoidCount: avoidIds.length }, items } satisfies ApiResponse,
      { status: 200 },
    )
  } catch {
