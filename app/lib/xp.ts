...
export default function MinhasConquistasPage() {
  ...
  const handleMissionToggle = (id: string) => {
    ...
    // Atualiza XP global e os números do painel
    const updated = updateXP(delta)
    setXp(updated)
  }
  ...
  const xpProgressPercent = totalXp === 0 ? 0 : Math.min(100, (todayXp / 400) * 100) // 400 é só um exemplo
