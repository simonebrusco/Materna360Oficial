...
export async function listIdeas(args: {
  ...
  let query = supabase.from('adm_ideas').select('*').order('updated_at', { ascending: false })
...
export async function getIdea(id: string) {
  ...
  const { data, error } = await supabase.from('adm_ideas').select('*').eq('id', id).single()
...
export async function updateIdea(id: string, patch: Partial<Omit<AdmIdeaRow, 'id' | 'created_at' | 'updated_at'>>) {
  await requireAdmin()
  const supabase = supabaseServer()

  const { error } = await supabase.from('adm_ideas').update(patch).eq('id', id)
