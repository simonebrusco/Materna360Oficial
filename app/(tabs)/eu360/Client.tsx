...
function personaCopy(persona: PersonaId) {
  switch (persona) {
    case 'sobrevivencia':
      return { label: 'Modo sobrevivência', microCopy: 'Aqui a regra é simples: menos cobrança, mais respiro. Vamos pelo possível.' }
    case 'organizacao':
      return { label: 'Organização leve', microCopy: 'Pequenos ajustes que tiram peso da rotina — sem te exigir perfeição.' }
    case 'conexao':
      return { label: 'Conexão', microCopy: 'Mais presença com menos esforço: 5 minutos intencionais já mudam o clima.' }
    case 'equilibrio':
      return { label: 'Equilíbrio', microCopy: 'Você está encontrando ritmo. Vamos manter constância gentil e clareza.' }
    case 'expansao':
      return { label: 'Expansão', microCopy: 'Você tem energia para avançar. Vamos aprofundar com escolhas mais conscientes.' }
  }
}
...
export default function Eu360Client() {
  ...
  useEffect(() => {
    ...
    const loadInsight = async () => {
      ...
      try {
        const result = await fetchWeeklyInsight({ firstName, stats })
  ...
  const content = (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(820px_520px_at_78%_22%,rgba(253,37,151,0.26)_0%,rgba(253,37,151,0.00)_62%)]"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-x-0 top-0 h-20 bg-[linear-gradient(to_bottom,rgba(255,255,255,0.18),rgba(255,255,255,0.00))]"
      />

      <div className="relative z-10 mx-auto max-w-3xl px-4 md:px-6">
        {/* TOP CARD (no padrão Meu Dia/Maternar) */}
        <div className="pt-8 md:pt-10">
          <SoftCard className="rounded-3xl border border-white/30 bg-white/10 backdrop-blur-md px-5 py-5 md:px-7 md:py-6 shadow-[0_18px_40px_rgba(0,0,0,0.18)]">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <span className="inline-flex items-center rounded-full border border-white/35 bg-white/12 px-3 py-1 text-[10px] font-semibold tracking-[0.24em] text-white uppercase">
                  EU360
                </span>

                <h1 className="mt-3 text-2xl md:text-3xl font-semibold text-white leading-tight drop-shadow-[0_2px_10px_rgba(0,0,0,0.38)]">
            ...
            <div className="mt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              {personaResult ? (
                ...
                <div className="rounded-2xl border border-white/30 bg-white/10 px-4 py-3">
                  ...
                  <p className="mt-1 text-[12px] text-white/85 leading-relaxed">
                    Responda 6 perguntas rápidas e o Eu360 calibra o ritmo do app para você.
                  </p>
                </div>
              )}
        ...
        <div className="mt-6 md:mt-7 space-y-6 md:space-y-7 pb-8">
          {/* 1 — PERFIL */}
          <ProfileForm />
