  ...
  return (
    <PageTemplate {...(heroProps as any)}>
      <SectionWrapper className="mx-auto max-w-5xl px-4 py-8 md:py-10">
        {/* CONTAINER PRINCIPAL – layout em colunas, para evitar blocos empilhados */}
        <div className="rounded-[32px] border border-white/70 bg-white/18 backdrop-blur-2xl shadow-[0_22px_55px_rgba(0,0,0,0.18)] px-4 py-5 md:px-7 md:py-7 space-y-5 md:space-y-6">
          {/* Mensagem de atalho / contexto (faixa leve no topo) */}
          {shortcutLabel && (
            <div className="rounded-2xl bg-white/90 border border-[var(--color-border-soft)] px-4 py-3 text-xs md:text-sm text-[#545454] shadow-[0_4px_16px_rgba(0,0,0,0.06)]">
              Você chegou aqui pelo atalho{' '}
              <strong>{shortcutLabel}</strong>. Que tal começar por essa área?
            </div>
          )}

          {/* Cabeçalho interno do hub */}
          ...
          {/* GRID PRINCIPAL – esquerda: Profissionais / direita: Comunidade + Serviços */}
          <div className="md:grid md:grid-cols-12 md:gap-6 lg:gap-8 md:items-start">
            {/* COLUNA ESQUERDA – PROFISSIONAIS (destaque) */}
            <section
              id="profissionais"
            ...
            >
              ...
              {/* Filtros – estilo Biblioteca Materna */}
              <div className="flex flex-wrap gap-2 pt-1">
                {[
                  ...
                  <span
                    key={tag}
                    className="inline-flex items-center rounded-full border border-[var(--color-border-soft)] bg-white/92 px-3 py-1 text-xs font-medium text-[#545454]"
                  >
                    {tag}
                  </span>
                ))}
              ...
              {/* Lista de profissionais – cards premium em coluna contínua */}
              <div className="mt-3 space-y-3">
                {PROFESSIONALS.map((prof) => (
                  <div
                    key={prof.id}
                    className="relative rounded-3xl border border-[var(--color-border-soft)] bg-white/95 shadow-[0_10px_26px_rgba(0,0,0,0.10)] px-4 py-4 md:px-5 md:py-4 flex flex-col gap-3"
                  >
                    ...
                    <button
                      ...
                      className="absolute top-3.5 right-3.5 inline-flex h-7 w-7 items-center justify-center rounded-full border border-[#FFD3E6] bg-white/90 text-[#FF005E] shadow-[0_4px_12px_rgba(0,0,0,0.08)]"
                    >
            ...
            {/* COLUNA DIREITA – COMUNIDADE + SERVIÇOS (painel lateral) */}
            <div className="mt-6 md:mt-0 md:col-span-5 lg:col-span-4 space-y-4">
              {/* Comunidade */}
              <section
                id="comunidade"
              ...
              >
                ...
                <SoftCard className="flex flex-col gap-3 p-4 md:p-5 bg-white/96">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-[#2F3A56]">
              ...
              >
                ...
                <SoftCard className="p-4 md:p-5 bg-white/96 space-y-3">
                  <p className="text-sm text-[#545454]">
