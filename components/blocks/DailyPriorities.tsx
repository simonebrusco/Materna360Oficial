...
export function DailyPriorities() {
  ...
  return (
    <SoftCard className="rounded-3xl border border-[#F5D7E5] bg-white p-5 md:p-6 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          {/* Eyebrow / título pequeno em rosa primário */}
          <p className="text-xs md:text-sm font-semibold uppercase tracking-wide text-[#fd2597]">
            Prioridades do Dia
          </p>
          {/* Body em texto principal */}
          <p className="mt-1 text-xs md:text-sm text-[#545454]">
            Escolha até três coisas que realmente importam para hoje.
          </p>
        </div>
        {/* Ícone em Plum / Ameixa (cor premium de ícones de card) */}
        <AppIcon
          name="checklist"
          className="h-5 w-5 text-[#b8236b] md:h-6 md:w-6"
          decorative
        />
      </div>
      ...
      <div className="space-y-3">
        {priorities.map(priority => (
          <div
            key={priority.id}
            className="flex items-center gap-3 rounded-2xl border border-[#F5D7E5] bg-white px-3 py-2.5 md:px-4 md:py-3"
          >
            {/* Checkbox estilizado */}
            <button
              type="button"
              ...
              className={`flex h-5 w-5 items-center justify-center rounded-full border text-xs transition-colors ${
                priority.completed
                  ? 'border-[#fd2597] bg-[#fd2597] text-white'
                  : 'border-[#F5D7E5] bg-white text-transparent'
              }`}
            >
              ✓
            </button>
            ...
            <input
              ...
              className="flex-1 border-none bg-transparent text-xs md:text-sm text-[#545454] placeholder:text-[#A0A0A0] focus:outline-none"
            />
          </div>
        ))}
      </div>
