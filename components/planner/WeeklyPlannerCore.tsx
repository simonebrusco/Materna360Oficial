...
function buildWeekData(baseDate: Date, plannerData: PlannerData): WeekDaySummary[] {
  ...
  // tasks são do selectedDateKey (persistência atual), então no WeekView
  // mostramos o que temos disponível para a semana como “resumo leve”.
  // Mantemos simples: contar por origin no dia selecionado.
  ...
  return weekDates.map(d => {
    ...
    return {
      ...
      top3Count: key === getBrazilDateKey(baseDate) ? taskCountByKey.top3 : 0,
      careCount: key === getBrazilDateKey(baseDate) ? taskCountByKey.selfcare : 0,
      familyCount: key === getBrazilDateKey(baseDate) ? taskCountByKey.family : 0,
    }
  })
}
  ...
  return (
    <Reveal>
      <div className="space-y-6 md:space-y-8">
        ...
        {/* WEEK VIEW (sempre pode aparecer, mas você controla pelo viewMode abaixo) */}
        {viewMode === 'week' && <WeekView weekData={weekData} />}

        {/* DAY VIEW (simples, sem modal ainda) */}
        {viewMode === 'day' && (
          <SoftCard className="rounded-3xl bg-white/95 border border-[var(--color-soft-strong)] p-4 md:p-6 space-y-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => addAppointment({ dateKey: selectedDateKey, title: 'Novo compromisso', time: '' })}
              ...
              {plannerData.tasks.length === 0 ? (
                ...
                plannerData.tasks.map(task => (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-[var(--color-text-muted)]">{task.origin}</span>
                  </button>
                ))
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button
                type="button"
                onClick={() => addTask('Minha prioridade 1', 'top3')}
              ...
              <button
                type="button"
                onClick={() => addAppointment({ dateKey: selectedDateKey, title: 'Compromisso', time: '' })}
                className="rounded-2xl bg-white/80 border border-[var(--color-soft-strong)] px-3 py-3 text-sm font-semibold text-[var(--color-text-main)] hover:border-[var(--color-brand)]/60"
              >
                Agenda
              </button>
              <button
                type="button"
                onClick={() => addTask('Um cuidado comigo', 'selfcare')}
                className="rounded-2xl bg-white/80 border border-[var(--color-soft-strong)] px-3 py-3 text-sm font-semibold text-[var(--color-text-main)] hover:border-[var(--color-brand)]/60"
              >
                Cuidar de mim
              </button>
              <button
                type="button"
                onClick={() => addTask('Um cuidado com meu filho', 'family')}
                className="rounded-2xl bg-white/80 border border-[var(--color-soft-strong)] px-3 py-3 text-sm font-semibold text-[var(--color-text-main)] hover:border-[var(--color-brand)]/60"
              >
                Cuidar do meu filho
              </button>
